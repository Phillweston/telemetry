...
//extern bool emulation_enable;

#define MAVLINK_RECEIVER_WARN(s, ...)		{printf(s, ## __VA_ARGS__);}
#define MAVLINK_RECEIVER_DEBUG(s, ...)		{printf(s, ## __VA_ARGS__);}
...
MavlinkReceiver::MavlinkReceiver(Mavlink *parent) :
	...
	_mom_switch_state(0)
{
	_handle_sys_rc_src = param_find("SYS_RC_SRC");
	param_get(_handle_sys_rc_src, &_sys_rc_src);
}
...
{
	switch (msg->msgid) {
	case MAVLINK_MSG_ID_COMMAND_LONG:				//地面站指令消息
...
{
	//printf_vx("[mavlink_receiver]handle_message_command_long...\n");
	/* command */
	...
	if (cmd_mavlink.target_system == mavlink_system.sysid && ((cmd_mavlink.target_component == mavlink_system.compid)
		...
		if (cmd_mavlink.command == MAV_CMD_PREFLIGHT_REBOOT_SHUTDOWN && ((int)cmd_mavlink.param1) == 3) {
			...
			MAVLINK_RECEIVER_WARN("terminated by remote");
			//fflush(stdout);
			taskDelay(50);
			...
			if (msg->sysid == mavlink_system.sysid && msg->compid == mavlink_system.compid) {
				MAVLINK_RECEIVER_WARN("ignoring CMD with same SYS/COMP (%d/%d) ID",
			...
			//MAVLINK_RECEIVER_WARN("receive msg 76 cmd %u",vcmd.command);
...

	{
      ...
      memcpy((char *)&ret_msg, (char *)msg,sizeof(mavlink_message_t));	
	  
	  ret_msg.payload64[32]|= 0x80;
	  _mavlink->send_message(MAVLINK_MSG_ID_COMMAND_LONG, &ret_msg);
...
MavlinkReceiver::handle_message_hil_optical_flow(mavlink_message_t *msg)
{
	...
	r.error_count = 0;
	r.type = range_finder_report::RANGE_FINDER_TYPE_LASER;
		...
		//if (_mavlink->get_forward_externalsp()) 
		  {
			...
			if (_control_mode.flag_control_offboard_enabled) {
				if (is_force_sp && offboard_control_mode.ignore_position &&
					...
					if (!offboard_control_mode.ignore_attitude && !math::isnan(set_position_target_local_ned.yaw)) {
					...
					if (!offboard_control_mode.ignore_bodyrate && !math::isnan(set_position_target_local_ned.yaw)) {
...
MavlinkReceiver::handle_message_rc_channels_override(mavlink_message_t *msg)
{
	...
	if (_rc_pub == nullptr) {
		_rc_pub = orb_advertise( (const struct orb_metadata *)ORB_ID(input_rc), &rc);

	} else {
		orb_publish( (const struct orb_metadata *)ORB_ID(input_rc), _rc_pub, &rc);
	}
}
...
MavlinkReceiver::handle_message_manual_control(mavlink_message_t *msg)
{
	...
	if (_mavlink->get_manual_input_mode_generation()) {
		...
		manual.regular_switch = decode_switch_pos(man.buttons, 2);
		manual.lcircle_switch = decode_switch_pos(man.buttons, 3);
...
MavlinkReceiver::handle_message_system_time(mavlink_message_t *msg)
{
	...
	if (!onb_unix_valid && ofb_unix_valid) {
		...
		if(clock_settime(CLOCK_REALTIME, &tv)) {
		    warn_vx("failed setting clock");
		}
		else {
		    warn_vx("[timesync] UTC time synced.");
		}
	}
...
MavlinkReceiver::handle_message_timesync(mavlink_message_t *msg)
{
	...
	if (tsync.tc1 == 0) {
		...
		if (dt > 10000000LL || dt < -10000000LL) { // 10 millisecond skew
			...
			MAVLINK_RECEIVER_WARN("[timesync] Hard setting offset.");
		} else {
...
MavlinkReceiver::handle_message_hil_sensor(mavlink_message_t *msg)
{
	...
	/* gyro */
	{
		...
		if (_gyro_pub == nullptr) {
			_gyro_pub = orb_advertise( (const struct orb_metadata *)ORB_ID(sensor_gyro), &gyro);

		} else {
			orb_publish( (const struct orb_metadata *)ORB_ID(sensor_gyro), _gyro_pub, &gyro);
		}
	}
	...
	/* accelerometer */
	{
		...
		if (_accel_pub == nullptr) {
			_accel_pub = orb_advertise( (const struct orb_metadata *)ORB_ID(sensor_accel), &accel);

		} else {
			orb_publish( (const struct orb_metadata *)ORB_ID(sensor_accel), _accel_pub, &accel);
		}
	}
	...
	/* magnetometer */
	{
		...
		if (_mag_pub == nullptr) {
			...
			_mag_pub = orb_advertise( (const struct orb_metadata *)ORB_ID(sensor_mag), &mag);

		} else {
			orb_publish( (const struct orb_metadata *)ORB_ID(sensor_mag), _mag_pub, &mag);
		}
	}
	...
	/* baro */
	{
		...
		if (_baro_pub == nullptr) {
			_baro_pub = orb_advertise( (const struct orb_metadata *)ORB_ID(sensor_baro), &baro);

		} else {
			orb_publish( (const struct orb_metadata *)ORB_ID(sensor_baro), _baro_pub, &baro);
		}
	}
	...
	if ((timestamp - _old_timestamp) > 10000000) {
		// printf_vx("receiving HIL sensors at %d hz\n", _hil_frames / 10);
...
MavlinkReceiver::handle_message_hil_gps(mavlink_message_t *msg)
{
	...
	hil_gps.cog_rad = _wrap_pi(gps.cog * 0.01745329251994f * 1e-2f);
...
{
	int uart_fd_main = _mavlink->get_uart_fd();
	int uart_fd_backup = _mavlink->get_uart_fd_back ();

		
	int _socket = _mavlink->get_udp_socket();
	const int timeout = 500;
	uint32_t	cnt_link_lost=0;
	uint8_t buf[100];
	float task_period=0.005f;
	...
	ssize_t nread = 0;
	//printf_vx("xxxx %d xxxxxxx ", uart_fd);
	while (!_mavlink->_task_should_exit) 
	{
		if (1)
		{
			...
			taskDelay(5);
			/*write(uart_fd, "dsafdasfdasfgfddg", 20);*/
			
			//write(uart_fd, "12345679", nread);
			orb_check(_vehicle_status_sub,&v_status_updated);
			if(v_status_updated)
			{
				struct vehicle_status_s ;
				orb_copy(ORB_ID(vehicle_status), _vehicle_status_sub, &_v_status);
...
/************主备链路切换管理********************/
			if(true==_v_status.data_link_lost)
			{
				cnt_link_lost++;
				if(cnt_link_lost*task_period>2.0f)
				{
					if(_tele_src_inuse==TELE_SRC_BACK)
						_tele_src_inuse=TELE_SRC_MAIN;
					else if(_tele_src_inuse==TELE_SRC_MAIN)
						_tele_src_inuse=TELE_SRC_BACK;
			...
			if(_tele_src_inuse==TELE_SRC_BACK)
			{
				_uart_fd_inuse = uart_fd_backup;
				_uart_fd_backup = uart_fd_main;
			}
			else
			{
				_uart_fd_inuse = uart_fd_main;
				_uart_fd_backup = uart_fd_backup;
			}

			clear_tele_backup_data(_uart_fd_backup);
/******************************************************************/			

			nread = read(_uart_fd_inuse, (char*)buf, sizeof(buf));
			//write(uart_fd, (char*)buf, nread);
#else
			nread = UdpRecvDataNoBlock(_socket, (char*)buf, sizeof(buf));
#endif
		#if 0
			int i = 0 ;
			while(i<nread)
			...
			for (ssize_t i = 0; i < nread; i++)
			{
				...
				{
					if(0xffff == _mavlink->_last_msg_seq)
					{
						_mavlink->_last_msg_seq = msg.seq;
					}
					else
					{ 
						uint32_t lost_bytes_cnt = 0;
						uint32_t current_seq = msg.seq;
						
						if(current_seq > _mavlink->_last_msg_seq)
						{
							lost_bytes_cnt = current_seq - _mavlink->_last_msg_seq - 1;
						}
						else
						{
							lost_bytes_cnt = current_seq + 256 - _mavlink->_last_msg_seq -1;
						}			
						_mavlink->count_rxLostMsgs(lost_bytes_cnt);
						
						_mavlink->_last_msg_seq = msg.seq;
					}
					_mavlink->count_rxMsgs(1);

					//warn_vx("----handle_message----");
					
					/* handle generic messages and commands */
					handle_message(&msg);/*普通消息*/

					/* handle packet with parent object */
					_mavlink->handle_message(&msg);/*mission,ftp,rtk,参数列表的处理*/
				}
				else
				{
					if (status.msg_received == MAVLINK_FRAMING_BAD_CRC)
					{
						_mavlink->count_rxCrcFail(1);
					}
				}
			}
...
void MavlinkReceiver::clear_tele_backup_data(int uart_fd)
{
	int i=0;
	...
	nread = read(uart_fd, (char*)buf, sizeof(buf));

	//若要检查备链数据的内容或频率,可继续执行解帧程序.
#if 0
	for (i = 0; i < nread; i++)
	{
		if (mavlink_parse_char(_mavlink->get_channel(), buf[i], &msg, &status))
...
MavlinkReceiver::receive_start(Mavlink *parent)
{
	...
	param.sched_priority = TASK_PRIO_MAVLINK_RECEIVE;	
	(void)pthread_attr_setschedparam(&receiveloop_attr, &param);
	pthread_attr_setname(&receiveloop_attr, (char*)"mavlink_recv");

	pthread_attr_setstacksize(&receiveloop_attr, 4096);
	pthread_t thread;
	pthread_create(&thread, &receiveloop_attr, MavlinkReceiver::start_helper, (void *)parent);

	pthread_attr_destroy(&receiveloop_attr); //线程摧毁但是资源并未释放，在mavlink_main中使用pthread_join()函数进行资源回收
	return thread;
}
