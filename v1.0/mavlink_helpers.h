...
/* Support shorter buffers than the
   default maximum packet size */
	...
	r_mavlink_status->msg_received = status->msg_received;// chj add this to make bad crc visibility
...
{
	return mavlink_frame_char_buffer((&m_mavlink_buffer[chan]),
...
MAVLINK_HELPER uint8_t mavlink_parse_char(uint8_t chan, uint8_t c, mavlink_message_t* r_message, mavlink_status_t* r_mavlink_status)
{
    ...
    if (msg_received == MAVLINK_FRAMING_BAD_CRC) {
	    ...
	    mavlink_message_t* rxmsg = (&m_mavlink_buffer[chan]);
	    mavlink_status_t* status = (&m_mavlink_status[chan]);
	    status->parse_error++;
	    status->msg_received = MAVLINK_FRAMING_INCOMPLETE;
	    status->parse_state = MAVLINK_PARSE_STATE_IDLE;
	    if (c == MAVLINK_STX)
